...
char rpn_col[RPN_MAX_SIZE][EXP_MAX_SIZE] = {0};
...
int resources_init(coap_context_t *context){
    ...
    coap_resource_set_observable(resource, 0);
...
void hnd_get(
    ...
    if( resource == coap_get_resource_from_uri_path(session->context, coap_make_str_const("rpn")) ){
		...
		else
		{
			...
			strcpy(query_bufor, (char *) query->s);
			char * bufor_n;
			//Reading n variable
...
void hnd_put(
	...
	if( resource == coap_get_resource_from_uri_path(session->context, coap_make_str_const("rpn")) )
	{
		...
		// Otherwise, try to parse data
			...
			strcpy(rpn_col[rpn_expression_count], (char *) data);
