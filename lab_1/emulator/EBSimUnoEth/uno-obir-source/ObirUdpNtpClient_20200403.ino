/*  Program demonstracyjny "OBIR NTP client", bazuje na UdpNtpClient dostarczanym z biblioteka "Arduino Ethernet"
 *
 *  Opis:
...
#include <ObirFeatures.h>       //for: ObirMilis()

//prototypy 
void ShowNtpResponse(byte *packetBuffer, int n);
void sendNTPpacket(ObirIPAddress address_ip);

/*
//method to access to particular fields in new_feature I/O Space
uint32_t read_new_feature_milis(){
    uint32_t t;
    t=(uint32_t)(_SFR_IO8(0x36))<<24;
...
//numer IP serwera NTP o FQDN: time.coi.pw.edu.pl
ObirIPAddress timeServer_ip=ObirIPAddress(194,29,130,252);
...
#define NTP_CLIENT_PERIOD       (60000UL)   //60sek - prosze zwrocic uwage na "UL" na koncy - niezbedne gdy chcemy aby stala byla typu 'unsigned long'
...
//    unsigned long t1=ObirMilis()+d;
//    for(;;)
//        if(t1<ObirMilis())
//            break;
//}

#define NTP_MACHINE_STATE_IDLE        0
#define NTP_MACHINE_STATE_SENT_REQ    1
int ntp_comm_state=NTP_MACHINE_STATE_IDLE;//nic nie slano
void setup() {
    //Zwyczajowe przywitanie z userem (niech wie ze system sie uruchomil poprawnie)
    Serial.begin(115200);
    Serial.print(F("obir eth ntp client init... ["));Serial.print(F(__FILE__));
    ...
    new_ntp_communications=ObirMilis();         //zadanie od serwera NTP ma sie odbyc juz na poczatku
...
void loop(){
    ...
    if(ntp_comm_state==NTP_MACHINE_STATE_IDLE){
        if(new_ntp_communications<ObirMilis()){                             //is time for a new NTP server request?
            new_ntp_communications=ObirMilis()+NTP_CLIENT_PERIOD;           //time when send a next new request
    ...
    ObirEthernet.maintain();        //zasadniczo odnawianie adresu pobranego z DHCP - proforma dla EBSimUnoEth'a 
}
